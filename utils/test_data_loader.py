...
class BraTSDataset(Dataset):
    """
    Dataset class for loading BraTS data directly from NIFTI files
    """
    ...
    def __init__(self, data_root, train_dataset, labels=False, smart_crop=False, custom_crops_file=None):
        ...
        Args:
            ...
            train_dataset (str): Dataset type (brats2020, brats2021, or brats2023)
            labels (bool): Whether to load label masks
            smart_crop (bool): Whether to use smart cropping for unlabeled data
            custom_crops_file (str): Path to file containing custom crop coordinates
        """
    ...
    def load_custom_crops(self, file_path):
        """
        Load custom crop coordinates from a file

        Args:
            file_path (str): Path to the file containing custom crop coordinates
        """
    ...
    def crop_image(self, image, mask=None, tumor_center=None, filename=None):
        """
        Crop image to 128x128x128 using various strategies

        Args:
            image: Input 3D image
            mask: Segmentation mask (for labeled data)
            tumor_center: Estimated tumor center (for smart cropping)
            filename: Original image filename (for custom crops)

        Returns:
            Cropped image and crop coordinates (start points)
        """
    ...
    def get_file_patterns(self):
        ...
        if self.train_dataset == 'brats2020':
            return {
                't1': '*t1.nii.gz',
                't1ce': '*t1ce.nii.gz',
                't2': '*t2.nii.gz',
        ...
        elif self.train_dataset == 'brats2019':
            return {
                't1': '*t1.nii',
                't1ce': '*t1ce.nii',
                't2': '*t2.nii',
