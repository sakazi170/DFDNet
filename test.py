...
from models.DFDNetworks import (base, base_EMF, base_MAX_EMF, base_ECB, base_ECB_EMF,
                                base_EMF_EFA, base_ECB_EMF_EFA, base_BIG, base_ECB_small,
                                base_without_bNeck, base_MAX, base_ECB_wo_evi, base_EMF_wo_entropy)
...
Checkpoint_path = "/home/zrway/qazisami/models/my_model_1/checkpoints/brats2020/base_ECB_EMF/train_only/base_ECB_EMF_350_loss_0.1744.pkl"

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', default="brats2020", help='dataset name')
    parser.add_argument('--model', type=str, default='DFDNet', help='network for training')
...
def main():
    ...
    model_dict = {
        'base': base,
        'base_EMF': base_EMF,
        'base_MAX_EMF': base_MAX_EMF,
        'base_ECB': base_ECB,
        'base_ECB_small': base_ECB_small,
        'base_ECB_EMF': base_ECB_EMF,
        'base_EMF_EFA': base_EMF_EFA,
        'base_ECB_EMF_EFA': base_ECB_EMF_EFA,
        'base_BIG': base_BIG,
        'base_without_bNeck': base_without_bNeck,
        'base_MAX': base_MAX,
        'base_ECB_wo_evi': base_ECB_wo_evi,
        'base_EMF_wo_entropy': base_EMF_wo_entropy
    }
    ...
    if args.save_pred:
        pred_savedir = f'./predictions/{args.dataset}/{args.model}/submission'
        ...
        if args.labels:
            ...
            for batch_idx, (t1, t1ce, t2, flair, true_mask, filename, crop_coords) in enumerate(dataloader):
                ...
                case_results.append({
                    ...
                    'hd95_et': et_metrics[1],  # Store actual value (including INF_VALUE)
        ...
        with open(csv_filename, 'w', newline='') as csvfile:
            ...
            for result in case_results:
                writer.writerow([
                    ...
                    f"{result['hd95_et']:.2f}",  # Will save INF_VALUE (373.13) as number
                    f"{result['hd95_tc']:.2f}",
                    f"{result['hd95_wt']:.2f}",
                    f"{result['hd95_avg']:.2f}"
                ])
